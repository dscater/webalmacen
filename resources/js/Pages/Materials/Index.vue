...
const { getMaterialsApi, setMaterial, limpiarMaterial, deleteMaterial } =
    useMaterials();
const responseMaterials = ref([]);
...
const loadItems = async ({ page, itemsPerPage, sortBy }) => {
    ...
    setTimeOutLoadData = setTimeout(async () => {
        responseMaterials.value = await getMaterialsApi(options.value);
        listMaterials.value = responseMaterials.value.data;
        totalItems.value = parseInt(responseMaterials.value.total);
        loading.value = false;
    }, 300);
};
const recargaMaterials = async () => {
    loading.value = true;
    listMaterials.value = [];
    options.value.search = search.value;
    responseMaterials.value = await getMaterialsApi(options.value);
...
const eliminarMaterial = (item) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            let respuesta = await deleteMaterial(item.id);
...
<template>
    ...
    <v-container>
                ...
                <v-card flat>
                    <v-card-title>
                        <v-row class="bg-primary d-flex align-center pa-3">
                            <v-col cols="12" sm="6" md="4"> Materiales </v-col>
        ...
        <Formulario
            ...
            @envio-formulario="recargaMaterials"
